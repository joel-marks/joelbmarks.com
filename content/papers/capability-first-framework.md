...
```mermaid
quadrantChart
    title Where does the organisation sit?
    x-axis Governance absent --> Governance structured
    y-axis Capability latent --> Capability active
    quadrant-1 Resourceful adoption
    quadrant-2 Shadow adoption
    quadrant-3 Dormant
    quadrant-4 Constrained
    Target state: [0.80, 0.82]
    Creatives, comms, coders: [0.24, 0.74]
    Management, administration: [0.50, 0.22]
```
...
```mermaid
flowchart TD
    A([Surface capability and need]) --> B[Adapt policy to function]
    B --> C[Develop capability by role]
    C --> D[Make knowledge AI-ready]
    D --> E[Govern by role]
    E --> F[Align to values]
    F -->|tools shift, people learn, repeat| A
```
